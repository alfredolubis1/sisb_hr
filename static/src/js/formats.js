 ...
 *
 * @param {Object} value read from TigernixERP
...
instance.web.format_value = function (value, descriptor, value_if_empty) {
    ...
    switch (descriptor.widget || descriptor.type || (descriptor.field && descriptor.field.type)) {
        ...
        case 'float2_time':
            ...
            ftime = value.toFixed(11)
            console.log('ftime',ftime)
            ftimes = value
            console.log('ftimes',ftimes)
            //console.log('fvalue',ftime)
            if (ftime < 0) {
        ...
        case 'day_time': //Widget Add By Alfredo for Adjust to SISB Leave Report
            var pattern = '%02d - %02d:%02d';
            console.log('value = ', value)
            fday = value.toFixed(16)
            console.log('fday =', fday)
            var day = Math.floor(fday)

            var hour_float = (fday % 1) * 24;
            var hour = Math.floor(hour_float)
            var min = Math.round((hour_float % 1) * 60);
            if (min >= 60) {
                min = 0;
                hour = hour + 1
            }
            ...
            return _.str.sprintf(pattern, day, hour, min);
        case 'many2one':
...
instance.web.parse_value = function (value, descriptor, value_if_empty) {
    ...
    switch (descriptor.widget || descriptor.type || (descriptor.field && descriptor.field.type)) {
        ...
        case 'float2_time':
            ...
            var float2_time_pair = value.split(":");
            console.log('float2_time_pair = ',float2_time_pair)
            if (float2_time_pair.length != 3)
                return factor * instance.web.parse_value(value, {type: "float"});
            var hours = instance.web.parse_value(float2_time_pair[0], {type: "integer"});
            var minutes = instance.web.parse_value(float2_time_pair[1], {type: "integer"});
            var seconds = instance.web.parse_value(float2_time_pair[2], {type: "integer"});
        ...
        case 'day_time': //Widget Add By Alfredo for Adjust to SISB Leave Report
            ...
            if (rtime.length != 2)
                return factor * instance.web.parse_value(value, {type: "float"})
            var days =  instance.web.parse_value(day, {type: "integer"})
            var time_hour = instance.web.parse_value(rtime[0], {type: "integer"})
            var minute_time = instance.web.parse_value(rtime[1], {type: "integer"})
            console.log('all time = ', days, time_hour, minute_time)
            return factor * (days + (time_hour / 24) + (minute_time / (60 * 24)))
        case 'progressbar':
